            ...
            for c in sorted(dates[a][b]):
                if b == sorted(dates[a].keys())[-1]:
                    if c == sorted(dates[a][b].keys())[-1]:
                        if len(dates[a][b][c]) > 1:
                            today_files = dates[a][b][c]
                            last_today_file = today_files.pop()
                            print('{} {} {} {} rm +'.format(a, b, c, today_files))
                            gdriveDelete(today_files)
                            print('{} {} {} {} last in year need\n'.format(a, b, c, last_today_file))
                        else:
